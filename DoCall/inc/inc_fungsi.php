...
function url_regist(){
    $url_regist  = "http://".$_SERVER['SERVER_NAME'].dirname($_SERVER['SCRIPT_NAME']);
    return $url_regist;

}
function ambil_gambar($id_tulisan){
    global $koneksi;
    $sql1 = "select * from halamaninput where id = '$id_tulisan'";
...
function buat_link_halaman($id){
    global $koneksi;
    $sql1    = "select * from halamaninput where id = '$id'";
    $q1     = mysqli_query($koneksi,$sql1);
    $r1     = mysqli_fetch_array($q1);
...
function kirim_email($email_penerima, $nama_penerima,$judul_email,$isi_email){
    ...
    $email_pengirim     = "rosmawinda22@gmail.com";
    $nama_pengirim      = "rosmawinda";
    ...
    try {
        ...
        $mail->Password   = 'winda1705';                               //SMTP password
